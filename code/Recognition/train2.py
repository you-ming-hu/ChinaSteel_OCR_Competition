...
def focal_loss(label,raw_predict,smoothing,corpus_size,alpha = 10):
    ...
    sigmoid_predict = tf.nn.sigmoid(raw_predict)
    loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=smooth_one_hot_label, logits=raw_predict)
    loss = tf.pow(tf.abs(smooth_one_hot_label-sigmoid_predict),alpha) * loss
...
loss_metric_name = 'focal loss'
train_loss_metric = tf.keras.metrics.Mean(name=loss_metric_name)
val_loss_metric = tf.keras.metrics.Mean(name=loss_metric_name)
best_val_CE_metric_value = tf.Variable(0.)
val_metrics.append((val_loss_metric,best_val_CE_metric_value))
...
for e in range(total_epochs):
    for batch_data in train_data:
        ...
        with tf.GradientTape() as tape:
            ...
            loss = loss_func(sequence,raw_predict,label_smoothing,corpus_size)
